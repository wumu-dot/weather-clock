...
static esp_err_t i2c_write_cmd(uint8_t cmd)
{
    ...
    if (r != ESP_OK) ESP_LOGE(OLED_TAG, "cmd 0x%02X fail: %d", cmd, r);
...
static esp_err_t i2c_write_data(const uint8_t *data, size_t len)
{
    ...
    i2c_master_stop(h);
    esp_err_t r = i2c_master_cmd_begin(I2C_MASTER_PORT, h, pdMS_TO_TICKS(100));
    if (r != ESP_OK) ESP_LOGE(OLED_TAG, "data fail: %d", r);
    i2c_cmd_link_delete(h);
    return r;
}
...
void oled_init(void)
{
    ...
    if (ret != ESP_OK) { ESP_LOGE("oled", "i2c_param_config: %d", ret); return; }
    ret = i2c_driver_install(I2C_MASTER_PORT, I2C_MODE_MASTER, 128, 128, 0);
    if (ret != ESP_OK) { ESP_LOGE("oled", "i2c_driver_install: %d", ret); return; }
